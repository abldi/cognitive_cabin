...
def transcribe_chunk(model, chunk_file):
    segments, info = model.transcribe(chunk_file, length_penalty=0.1, beam_size=5, vad_filter=True,
...
if __name__ == "__main__":
    ...
    p = pyaudio.PyAudio()
    stream = p.open(input_device_index=9, format=pyaudio.paInt16, channels=1, rate=16000, input=True, frames_per_buffer=1024)
    ...
    try:
        while True:
            ...
            # audio_file_path = '/home/tamaya/Documents/cognitive_cabin/harvard.wav'
            # transcription = transcribe_chunk(model, audio_file_path)
