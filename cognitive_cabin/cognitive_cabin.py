...
class CognitiveCabin(cmd.Cmd):
    ...
    def __init__(self, interfaceArg ,start_mode="dev"):
        super().__init__()
        self.interface = interfaceArg
        self.streaming = True
        text_to_speech.mode = start_mode
        text_to_speech.synthesize("Welcome aboard to the Cognitive Cabin Software. "
    ...
    def process(self, images):
        ...
        try:
            ...
            print(cabin_assistant_response['message']['content'])
            self.interface.sendToInterface(cabin_assistant_response['message']['content'])
            self.streaming = True
            text_to_speech.synthesize(cabin_assistant_response['message']['content'],
                                      use_stream=True,
                                      stream_end_callback=self.audio_stream_finished).then().catch()
